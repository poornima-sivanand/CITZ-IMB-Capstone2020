...
static Map exec(List args, Appendable stdout=null, Appendable stderr=null, Closure stdin=null){
    ...
    def run() {
        ...
        Map ocGetSecretToken = exec(['sh', '-c', "set -x; oc get \"secret/\$(cat /var/run/secrets/bitbucket/metadata.name)\" \"--output=jsonpath={.data['generic-hook\\.token']}\" | base64 --decode"])

        if (ocGetSecretToken.status != 0 || ocGetSecretToken.out.toString().trim() == ""){
            println "Updating/Creating token"
            exec(['sh', '-c', "oc patch \"secret/\$(cat /var/run/secrets/bitbucket/metadata.name)\" -p '{\"stringData\": {\"generic-hook.token\": \"${secretToken}\"}}'" as String])
        }else{
